...
INTEREST_WEIGHT = 0.5
AVAILABILITY_WEIGHT = -0.2 # remove
NUMBER_MENTOR_WEIGHT = -0.2
NUMBER_MENTEE_WEIGHT = -0.4
LOCATION_WEIGHT = 0.4
CAMPUS_WEIGHT = 0.8
BUILDING_WEIGHT = 0.2
TIME_WEIGHT = 0.1
...
class MatchingPool:
    ...
    def match_list(self):
        ...
        if len(self.result) > 0:
            temp = [(w,self.result[w]) for w in sorted(self.result, key = self.result.get,reverse = True)]
##            print [i[0] for i in temp][:3]
            return [i[0] for i in temp][:3]
    def best_match(self):
...
class CalculateScore:       
    ...
    def calculate_score(self):
        ...
        time_score = compare(self.mentor.atWork, self.mentee.atWork)

        # number of mentee of a mentor give negative weight to the score
        availability = self.mentor.numMentees*NUMBER_MENTEE_WEIGHT + self.mentee.numMentors*NUMBER_MENTEE_WEIGHT
        
        self.score = INTEREST_WEIGHT*score + LOCATION_WEIGHT*location_score + availability + time_score*TIME_WEIGHT
        return self.score
